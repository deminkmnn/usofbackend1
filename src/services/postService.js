...
class PostService {
  // GET /api/posts
  async listPublic({ sort = 'likes', category, from, to, status }) {
    let query = `
      ...
      LEFT JOIN PostCategories pc ON pc.post_id = p.id
      WHERE 1=1
    `;
    const params = [];
    ...
    if (sort === 'date') {
      ...
      query += ' ORDER BY like_count DESC'; // за замовчуванням — по лайках
    }
