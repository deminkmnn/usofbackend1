...
/** РЕЄСТРАЦІЯ АДМІНОМ */
const RegistrationByAdmin = async (email, login, password, repeatedPassword, full_name = '', role = 'user') => {
  await checkEmailLoginUnique(email, login);
  if (password !== repeatedPassword) throw errorForApi.BadRequestError('Passwords do not match');
  const hashPassword = await bcrypt.hash(password, 4);
  const [result] = await pool.query(
    `INSERT INTO Users (email, login, password, full_name, role) VALUES (?, ?, ?, ?, ?)`,
    [email, login, hashPassword, full_name, role]
  );
  return { id: result.insertId, email, login, full_name, role };
};
